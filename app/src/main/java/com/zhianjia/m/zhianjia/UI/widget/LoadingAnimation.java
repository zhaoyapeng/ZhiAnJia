...
public class LoadingAnimation extends SurfaceView implements SurfaceHolder.Callback {

    private final float MOVE_SPEED = 90f;
    private final float MIN_SPEED = 40f;
    private final float A = 15f;
    private final float MAX_ANGLE = 340f;
    private final float DRAW_WIDTH = 3;
    ...
    private void calculateAnimation() {
        if (state == 0) {
            ...
            if (edAngle < halfMax) {
                ...
                edAngle = 0;
                edSpeed = 0;
                offsetAngle -= (360 - MAX_ANGLE);
            }
            edAngle += edSpeed * deltaTime;
        }
        offsetAngle += MOVE_SPEED * deltaTime;
        if (offsetAngle > 360) {
            offsetAngle -= 360;
        }
    }
